...
func (p *Service) AdminLogin(ctx context.Context, username string, password string) (usermodel.User, string, error) {
...
func (p *Service) UserLogin(ctx context.Context, phone string, password string) (usermodel.User, string, error) {
...
func (p *Service) Profile(ctx context.Context, raw string, scope string) (usermodel.User, error) {
	user, _, err := p.Current(ctx, raw, scope)
	return user, err
}

func (p *Service) Current(ctx context.Context, raw string, scope string) (usermodel.User, token.Claims, error) {
	claims, err := p.verify(ctx, raw, scope)
	if err != nil {
		return usermodel.User{}, token.Claims{}, err
	}
	userID, err := strconv.ParseUint(claims.Subject, 10, 64)
	if err != nil {
		return usermodel.User{}, token.Claims{}, ErrLogin
	}
	user, err := p.store.ByID(ctx, userID)
	return user, claims, err
}

func (p *Service) Refresh(ctx context.Context, raw string, scope string) (usermodel.User, token.Token, error) {
	user, claims, err := p.Current(ctx, raw, scope)
	if err != nil {
		return usermodel.User{}, token.Token{}, err
	}
...
func (p *Service) ProfileByID(ctx context.Context, id uint64) (usermodel.User, error) {
	return p.store.ByID(ctx, id)
}

func (p *Service) login(ctx context.Context, user usermodel.User, account usermodel.Account, password string, scope string) (usermodel.User, string, error) {
