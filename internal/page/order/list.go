...
func List() *viewer.Viewer[service.Order] {
	return viewer.New[service.Order]("订单列表").
		Identity("order.list").
		Resource(orderconstant.List).
		Columns(
		...
		Buttons(
			button.Open(
				...
				button.Resource(orderconstant.Update, permission.Update),
		...
		Paging(1, 30, 2).
		WithData(
			service.Order{ID: 1, Number: "NO-20260716-001", Status: "open", Amount: 12800},
			service.Order{ID: 2, Number: "NO-20260716-002", Status: "closed", Amount: 6800},
		)
}
