...
func (p Store) ByAdmin(ctx context.Context, username string) (User, Account, error) {
	return p.byAccount(ctx, useraccount.ProviderPassword, username, "admin")
}

func (p Store) ByPhone(ctx context.Context, phone string) (User, Account, error) {
	return p.byAccount(ctx, useraccount.ProviderPhone, phone, "user")
}

func (p Store) ByID(ctx context.Context, id uint64) (User, error) {
	var user User
...
func prepareDefaults(defaults []DefaultUser) ([]User, []string, int, error) {
	users := make([]User, len(defaults))
...
func saveDefaultUsers(tx *gorm.DB, users []User) error {
...
func defaultAccounts(defaults []DefaultUser, ids map[string]uint64, capacity int) ([]Account, error) {
	accounts := make([]Account, 0, capacity)
...
func saveDefaultAccounts(tx *gorm.DB, accounts []Account) error {
	if len(accounts) == 0 {
		return nil
	}
	err := tx.Clauses(clause.OnConflict{
...
func (p Store) byAccount(ctx context.Context, provider useraccount.Provider, identifier string, role string) (User, Account, error) {
	var account Account
	err := p.db.WithContext(ctx).
		Where("provider = ? AND identifier = ?", provider.String(), identifier).
		First(&account).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return User{}, Account{}, ErrLogin
	}
	if err != nil {
...
func (p Store) first(ctx context.Context, query string, args ...any) (User, error) {
	var user User
